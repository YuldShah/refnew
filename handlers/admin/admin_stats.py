...
async def show_admin_stats(message: Message, db: Database):
    ...
    async with db.pool.acquire() as conn:
        ...
        total_valid_referrals = await conn.fetchval('SELECT COUNT(*) FROM referrals WHERE valid = TRUE')
        total_pending_referrals = await conn.fetchval('SELECT COUNT(*) FROM referrals WHERE valid = FALSE')
